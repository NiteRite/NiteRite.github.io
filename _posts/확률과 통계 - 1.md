...
### 집합의 Basic Operation (Based on Boolean Algebra) ###
  ...
  * Complement(여집합) == A<sup>c: Opposite event, which consists of elements that are not members of A
...
### Theorem: For any sets A, B, C, we have - ###
  ...
  4. De Morgan's Law: (A ∩ B)<sup>c = A<sup>c ∪ B<sup>c // (A∪B)<sup>c = A<sup>c ∩ B<sup>c 
