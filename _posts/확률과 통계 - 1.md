...
### 집합의 Basic Operation (Based on Boolean Algebra) ###
  ...
  * Complement(여집합) == A^c: Opposite event, which consists of elements that are not members of A
...
해당 포스트에서는, 원래 여집합 기호는 (^c) 이지만, 마땅히 포스트 내에서 구현할 방법이 없어, Boolean Algebra의 Complement 기호인 (') 으로 대체하였습니다. 앞으로의 포스트에서 그럴 것입니다만, 손필기 혹은 공식 이미지에서는 정상적으로 (^c)가 나타날 것 같습니다.
...
### Basic Principles of Probability ###
  ...
  3. Sigma-Algebra: В를 표본 공간 S의 부분집합들의 모음이라고 할 때, 우리는 이러한 B를 σ-algebra 혹은 σ-field라고 한다. 이러한 B는 세 가지 성질을 가진다. 첫째, B는 Ø를 원소로 갖는다. 둘째, if A∈B, then A'∈B. 셋째, if A1, A2, A3...∈B, then ∪(i=1 to ∞) Ai∈B. 
