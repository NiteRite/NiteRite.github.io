...
### Theorem: For any sets A, B, C, we have - ###
  ...
  4. De Morgan's Law: (A ∩ B)^c^ = A^c^ ∪ B^c^ // (A∪B)^c^ = A^c^ ∩ B^c^ 
...
### Basic Principles of Probability ###
  ...
  3. Sigma-Algebra: В를 표본 공간 S의 부분집합들의 모음이라고 할 때, 우리는 이러한 B를 σ-algebra 혹은 σ-field라고 한다. 이러한 B는 세 가지 성질을 가진다. 첫째, B는 Ø를 원소로 갖는다. 둘째, if A∈B, then A^c^∈B. 셋째, if A1, A2, A3...∈B, then ∪(i=1 to ∞) Ai∈B. 
